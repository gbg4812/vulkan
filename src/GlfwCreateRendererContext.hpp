...
namespace gbg {
    ...
    uint32_t deviceScore(VkPhysicalDevice device, VkSurfaceKHR surface, const std::vector<const char*> deviceExtensions) {
    ...
    inline RendererContext glfwCreateRendererContext(GLFWwindow* window, const std::vector<const char*> & validationLayers,
        bool enableValidationLayers, const std::vector<const char*>& deviceExtensions) {
        RendererContext context{};
        context.instance = createInstance(validationLayers, enableValidationLayers);

        VkSurfaceKHR surface;
        if (glfwCreateWindowSurface(context.instance.instance, window, nullptr, &surface) !=
            VK_SUCCESS) {
        ...
        // physical device
        VkPhysicalDevice physicalDevice = pickPhysicalDevice(context.instance, surface, deviceExtensions);
        vkDevice device = createDevice(physicalDevice, deviceExtensions,enableValidationLayers, validationLayers, surface);
        return context;
    }
}
