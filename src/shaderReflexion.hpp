...
// error setShaderCode(shader, filepath, type); // read the file and compile, then assign the bytecode if no errors (clear last) (DONE)
// reflectShader(shader); // fill the shader from the bytecodes (clear and recreate parameters) (DONE)
// materialAssignShader(shaderh, scene); // clear then create the material parameters from the shader and assign (DONE IN MATERIAL)
...
namespace gbg {
    ...
    inline void reflectShader(Shader& shader) {
        ...
        spvReflectCreateShaderModule(shader.getVertShaderCode().size()*sizeof(uint32_t),
                                     shader.getVertShaderCode().data(), &vtmod);
    
        SpvReflectShaderModule fgmod;
        spvReflectCreateShaderModule(shader.getFragShaderCode().size()*sizeof(uint32_t),
    ...
    inline std::pair<bool, std::string> setShaderCode(gbg::Shader& sh, std::filesystem::path path, ShaderType type) {
        ...
        shaderc::CompilationResult res = cmp.CompileGlslToSpv(data.data(), kind, path.filename().c_str());
        ...
        return {res.GetCompilationStatus() == shaderc_compilation_status_success, res.GetErrorMessage()};
    }
